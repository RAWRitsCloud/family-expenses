...
function validatePayload(value) {
  ...
  value.expenses.forEach((expense, index) => {
    if ("james" in expense || "donal" in expense) {
      throw new Error(`Expense ${index + 1}: use paidBy instead of legacy payer fields.`);
    }
...
async function readFallbackLocalData() {
    ...
    try {
      ...
      if (request.method === "GET") {
        ...
        try {
          ...
          context.log(`GitHub read failed, falling back to local data: ${error.message}`);
          const localData = await readFallbackLocalData();
          return {
            status: 200,
            jsonBody: localData,
            headers: {
